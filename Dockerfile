...
# Setup the Cron Job
# 1. Copy the config file to the cron.d directory
# Create the cron file directly inside Linux to avoid Windows line-ending issues
RUN echo "* * * * * cd /app && /usr/local/bin/python3 scripts/log_2fa_cron.py >> /cron/last_code.txt 2>&1" > /etc/cron.d/2fa-cron && \
